...
from src.models.communication import Communication, CommunicationType
from src.agent.listing_agent import ListingAgent
from src.email.email_client import EmailClient
from src.scrapers.listing_project import ListingProject
...
class DwellApp:
    ...
    def display_listing_summary(self, listing: Listing, index: int, total: int):
        ...
        print(f"📅 Dates: {listing.start_date.strftime('%Y-%m-%d') if listing.start_date else 'N/A'} to {listing.end_date.strftime('%Y-%m-%d') if listing.end_date else 'N/A'}")
    ...
    def display_communication_history(self, listing_id: str):
        ...
        if history:
            ...
            for comm in history[:3]:  # Show last 3 communications
                status_emoji = "✅" if comm.is_sent() else "❌" if comm.has_failed() else "⏳"
                print(f"   {status_emoji} {comm.communication_type.value.upper()}: {comm.status.value} ({comm.generated_at.strftime('%Y-%m-%d %H:%M') if comm.generated_at else 'N/A'})")
        else:
    ...
    def get_user_decision(self) -> str:
        ...
        while True:
            ...
            print(f"   [S]end - Send this email")
            print(f"   [R]eject - Skip this listing")
            print(f"   [E]dit - Edit email (coming soon)")
            print(f"   [Q]uit - Exit application")
    ...
    def process_listings(self, max_listings: int = 30):
        ...
        print(f"🚀 Starting email approval process...")
        print(f"📊 Processing up to {max_listings} listings that haven't been emailed")
        
        # Get communication statistics
        stats = self.agent.get_communication_stats()
        print(f"📈 Communication Stats: {stats.get('total_communications', 0)} total, {stats.get('status_counts', {}).get('sent', 0)} sent")
        ...
        for listing in self.agent.get_candidate_listings():
            ...
            try:
                ...
                if decision == 'S':
                    ...
                    print(f"❌ Email rejected. Moving to next listing...")
                    
                elif decision == 'E':
                    # Edit email (placeholder for future implementation)
                    print(f"✏️  Edit feature coming soon! For now, rejecting...")
                    self.stats['rejected'] += 1
                ...
                if decision == 'S':
                    import time
                    print(f"⏱️  Waiting 2 seconds before next listing...")
                    time.sleep(2)
        ...
        # Show final count
        if processed == 0:
            print(f"✨ No candidate listings found! All matching listings have been contacted.")
        else:
