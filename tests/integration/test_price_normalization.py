...
class TestPriceNormalizationIntegration:
    ...
    def test_monthly_user_filtering(self, db_with_test_data):
        ...
        with get_db_session() as db:
            ...
            expected_ids = [
                "equiv_daily_1", "equiv_weekly_1", "equiv_monthly_1",
                "equiv_daily_2", "equiv_weekly_2", "equiv_monthly_2"
            ]
            
            assert len(candidates) == 6, f"Expected 6 candidates, got {len(candidates)}"
            assert set(candidate_ids) == set(expected_ids), f"Expected {expected_ids}, got {candidate_ids}"
    ...
    def test_cross_period_consistency(self, db_with_test_data):
        ...
        with get_db_session() as db:
            ...
            assert daily_ids == expected_ids, f"Daily user got {daily_ids}, expected {expected_ids}"
            assert weekly_ids == expected_ids, f"Weekly user got {weekly_ids}, expected {expected_ids}"
            assert monthly_ids == expected_ids, f"Monthly user got {monthly_ids}, expected {expected_ids}"
            
            # All should have same number of candidates
            assert len(daily_candidates) == len(weekly_candidates) == len(monthly_candidates)
    ...
    def test_date_flexibility_filtering(self, db_with_test_data):
        ...
        with get_db_session() as db:
            ...
            flexible_listing = Listing(
                ...
                price=3500.0,
                price_period=PricePeriod.MONTH,
                start_date=datetime(2025, 7, 28),  # 4 days before user's preferred start
            ...
            assert "flexible_listing" in candidate_ids, f"Flexible listing not found in {candidate_ids}"
