...
class TestListingPriceCalculations:
    ...
    def test_calculate_total_cost_weekly(self):
        ...
        assert listing.calculate_total_cost_for_duration(30) == pytest.approx(
            4500.0, rel=1e-2
        )  # ~1 month
...
class TestPriceEquivalence:
    ...
    def test_equivalent_prices_same_total_cost(self):
        ...
        for duration in durations:
            ...
            assert (
                abs(daily_cost - weekly_cost) < 0.01
            ), f"Daily vs Weekly mismatch for {duration} days"
            assert (
                abs(daily_cost - monthly_cost) < 0.01
            ), f"Daily vs Monthly mismatch for {duration} days"
            assert (
                abs(weekly_cost - monthly_cost) < 0.01
            ), f"Weekly vs Monthly mismatch for {duration} days"
    ...
    def test_different_equivalent_sets(self):
        ...
        assert (
            abs(daily_1_cost - daily_2_cost) > 100
