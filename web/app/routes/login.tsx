...
import { checkProfileCompletion } from "~/services/profile.server";

export async function loader({ request }: LoaderFunctionArgs) {
  const { user } = await getAuthenticatedUser(request);

  if (user) {
    const { isComplete } = await checkProfileCompletion(request);
    return redirect(isComplete ? "/dashboard" : "/chat");
  }
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const { isComplete } = await checkProfileCompletion(request);
  return redirect(isComplete ? "/dashboard" : "/chat", { headers });
}
